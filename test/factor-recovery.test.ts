...
test('recovery handlers require same-origin CSRF, never mutate on GET and issue enrollment-only cookies',async t=>{
 ...
 const instance=await authExtension({service,csrfKey,projectSha256:'a'.repeat(64),sendFactorRecovery:async message=>{delivered.push(message);}}).activate({registration:'open'},{origin,target:'node',projectSha256:'a'.repeat(64),mounts:['/account'], root: import.meta.dirname});
