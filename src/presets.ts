...
export interface AuthPreset {
    ...
    extension: Pick<AuthExtensionOptions, 'passkeys' | 'sendToken' | 'sendEmailCode' | 'sendNotice'>;
...
export function createAuthPreset(options: AuthPresetOptions): AuthPreset {
    ...
    if (options.sender && (typeof options.sender !== 'function' || typeof options.sender.notify !== 'function' || typeof options.sender.sendEmailCode !== 'function'))
    ...
    return { service: Object.freeze(service), extension: Object.freeze({ passkeys, ...(sender ? { sendToken: sender, sendEmailCode: sender.sendEmailCode.bind(sender), sendNotice: sender.notify.bind(sender) } : {}) }), notices: Object.freeze(sender ? [] : ['Email verification, recovery and notices are unavailable until an operator sender is configured.']) };
}
