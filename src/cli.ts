...
try {
    ...
    if (values.help || !command)
        ...
        if (command === 'init') {
            ...
            if (!['bootstrap', 'users', 'sessions', 'revoke', 'audit', 'import', 'rotate-key', 'purge', 'cleanup', 'doctor'].includes(command))
                throw new Error('Invalid command');
