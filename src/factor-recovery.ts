...
export interface FactorRecoveryMessage {email:string;verificationToken:string;cancelToken:string;signal:AbortSignal}
export interface FactorRecoveryOptions {service:FactorRecoveryService;sendFactorRecovery?:(message:FactorRecoveryMessage)=>Promise<void>}
...
export function createFactorRecoveryFlows(options:FactorRecoveryOptions,http:AuthHttp,mount:string){
 ...
 const form=(path:string,csrf:string,markup:string,label:string)=>`<form method="post" action="${escapeHtml(mount+path)}">${csrfField(csrf)}${markup}<button type="submit">${escapeHtml(label)}</button></form>`;
 ...
 return {enabled,async handle(request:ExtensionRequest,presentation:PresentationContext=createPresentation().resolve()):Promise<AuthHttpResponse|undefined>{
  ...
  const page=(title:string,markup:string,status=200,headers:[string,string][]=[])=>(pageResponse(title,markup,status,headers,undefined,presentation));
  ...
  if(request.method!=='POST'){
   ...
   if(path==='/recover-factor')return page(tr('recovery.title'),`<p>${escapeHtml(tr('recovery.intro'))}</p>`+form(path,prepared.csrf,formField('email','Email address','email','username'),tr('recovery.send')),200,prepared.headers);
  ...
  if(path==='/recover-factor'){
   ...
   if(issued.verificationToken&&issued.cancelToken){
    ...
    try{await Promise.race([options.sendFactorRecovery!({email:fields.email||'',verificationToken:issued.verificationToken,cancelToken:issued.cancelToken,signal:controller.signal}),new Promise<void>((_,reject)=>{timer=setTimeout(()=>{controller.abort();reject(new Error('Delivery timeout'));},5000);})]);}
